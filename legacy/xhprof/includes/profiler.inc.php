...
register_shutdown_function(function () use ($config) {
	...
	$queries = array_map(function ($e) use (&$total_duration) {
		$e['Duration'] = 1000000 * $e['Duration'];
		$total_duration += $e['Duration'];
		$e['Query'] = preg_replace('/\s+/', ' ', $e['Query']);
		return $e;
	}, $queries);
	...
	<style>
		...
		.mysql-debug-table table {
			width: 100%;
		}

		.mysql-debug-table th.id,
		.mysql-debug-table th.duration {
			width: 100px;
		}
	</style>
	<?php
